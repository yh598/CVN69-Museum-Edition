#!/usr/bin/env python3
"""Run Bambu Studio's independent import/manifold checks for Milestone 3."""
...
def main():
    ...
    files = sorted((ISLAND / "STL").glob("*.stl")) + sorted((ISLAND / "3MF").glob("*.3mf"))
    records = []
    with tempfile.TemporaryDirectory(prefix="cvn69_m3_bambu_") as temporary:
        for path in files:
            ...
            relative = str(path.relative_to(ISLAND))
    ...
    for record in records:
        ...
        lines.append(
            f"| `{record['path']}` | {record['status']}{suffix} | {record['manifold']} | "
