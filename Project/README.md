...
Milestone 2 hull–flight-deck integration is available for review under [`Integration/`](Integration/README.md). It combines the approved hull and deck packages with a concealed printed-pad, glue-only interface; all mandatory geometry, interference, mesh, package, and Bambu Studio checks pass. The 0.25 mm-per-side interface coupon also has a recorded physical PASS at the qualified print settings, and the production interface dimensions are frozen. It remains intentionally untagged pending reviewer approval.
